...
class Ovpn3Client(private val host: Ovpn3TunHost) : ClientAPI_OpenVPNClient() {
    ...
    // This openvpn3 rev delivers DNS via a single DnsOptions struct (not per-server add_dns_server /
    // add_search_domain callbacks). Best-effort no-op: the tun host (Ovpn3VpnService.Builder) sets a
    // sane default DNS, so the tunnel + name resolution work without navigating the DnsOptions struct.
    override fun tun_builder_set_dns_options(dns: DnsOptions?): Boolean = true
